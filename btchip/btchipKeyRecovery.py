...
class MyVerifyingKey(ecdsa.VerifyingKey):
    ...
    def from_signature(klass, sig, recid, h, curve):
        ...
        x3 = pow(x, 3, curveFp.p())
        ax = (curveFp.a() * x) % curveFp.p()
        alpha = (x3 + ax + curveFp.b()) % curveFp.p()
...
def point_to_ser(P):
    ...
    hy = '%064x' % P.y()
    hp = '04' + hx + hy
    # return ( '04'+()+('%064x'%P.y()) ).decode('hex')
    return bytearray.fromhex(hp)
...
def recoverKey(signature, hashValue, keyX):
    ...
    for i in range(4):
        try:
            ...
            key = MyVerifyingKey.from_signature(bytearray(sign), i, hashValue, curve=SECP256k1)
